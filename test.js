...
describe("Post", () => {
  it("Should create a new payment", async () => {
    ...
    const res = await chai.request(app).post("/payment").send(payment);
...
describe("Get", () => {
  before(async () => {
    const response = await chai.request(app).post("/payment").send(payment);

    createdPayment = response.body;
  });

  it("Should get all payments", async () => {
    const res = await chai.request(app).get("/payment");

    expect(res.body).to.be.an("array");
    expect(res.body.some((payment) => payment._id === createdPayment._id)).to.be
      .true;
  });
